...
function publicJob(job) {
  return { id: job.id, url: job.url, slug: job.slug, title: job.title, key: job.key, assetKey: job.assetKey, icon: job.icon, iconUrl: job.iconUrl, iconOnly: Boolean(job.iconOnly), profileOnly: Boolean(job.profileOnly), status: job.status, stage: job.stage, error: job.error, archive: job.archive, files: job.files, protocolCapture: job.protocolCapture || '', profile: job.profile || '', logs: job.logs };
}
...
function validateProductUrl(input) {
  ...
  if (url.protocol !== 'https:' || !['egt-digital.com', 'www.egt-digital.com'].includes(url.hostname)) throw new Error('Only HTTPS egt-digital.com URLs are accepted');
  const match = url.pathname.match(/^\/game\/([a-z0-9-]+)\/?$/i);
  if (!match) throw new Error('URL must use /game/{slug}/');
  return { url: `https://egt-digital.com/game/${match[1].toLowerCase()}/`, slug: match[1].toLowerCase() };
}
...
async function discoverLinks(input) {
  ...
  try {
    ...
    for (let pass = 0; pass < 50; pass++) {
      ...
      const beforeGames = await page.evaluate(() => new Set([...document.querySelectorAll('a[href]')].map(anchor => anchor.href).filter(href => href.startsWith('https://egt-digital.com/game/'))).size);
      await more.scrollIntoViewIfNeeded();
      await more.click();
      const grew = await page.waitForFunction(previous => new Set([...document.querySelectorAll('a[href]')].map(anchor => anchor.href).filter(href => href.startsWith('https://egt-digital.com/game/'))).size > previous, beforeGames, { timeout: 10000 }).then(() => true).catch(() => false);
      if (!grew) break;
    }
    finalUrl = page.url();
  } catch (browserError) {
  ...
  if (!rawLinks.some(link => /(?:https?:\/\/(?:www\.)?egt-digital\.com)?\/game\/[a-z0-9-]+\/?/i.test(link.href || ''))) {
    ...
    rawLinks.push(...[...html.matchAll(/(?:https?:\/\/(?:www\.)?egt-digital\.com)?\/game\/([a-z0-9-]+)\/?/gi)].map(match => ({ href: `https://egt-digital.com/game/${match[1].toLowerCase()}/`, text: '' })));
  }
  ...
  for (const raw of rawLinks) {
    try {
      ...
      resolved.hash = '';
      const gameMatch = resolved.hostname.match(/^(?:www\.)?egt-digital\.com$/i) && resolved.pathname.match(/^\/game\/([a-z0-9-]+)\/?$/i);
      if (!gameMatch) continue;
      const normalized = `https://egt-digital.com/game/${gameMatch[1].toLowerCase()}/`;
      ...
      found.set(normalized, {
        ...
        processable: true,
        slug: gameMatch[1].toLowerCase(),
      });
      if (found.size >= 2000) break;
    } catch {}
  }
  const links = [...found.values()].sort((a, b) => a.url.localeCompare(b.url));
  if (!links.length) throw new Error('No https://egt-digital.com/game/ links were found on this page');
...
function createJob(parsed, options = {}) {
  return { id: `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`, ...parsed, status: 'queued', stage: options.iconOnly ? 'Queued · icon only' : options.profileOnly ? 'Queued · local profile' : 'Queued', logs: [], error: '', key: '', title: '', assetKey: '', icon: '', iconUrl: '', iconOnly: Boolean(options.iconOnly), profileOnly: Boolean(options.profileOnly), archive: '', files: 0, protocolCapture: '', profile: '', overwrite: Boolean(options.overwrite), captureSeconds: Math.min(300, Math.max(15, Number(options.captureSeconds) || 60)), managementSession: options.managementSession || null };
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url.pathname === '/api/import') {
    try {
      ...
      const created = inputs.map(input => { const job = createJob(validateProductUrl(input), { overwrite: body.overwrite, captureSeconds: body.captureSeconds, iconOnly: body.iconOnly, managementSession: session }); jobs.push(job); return publicJob(job); });
