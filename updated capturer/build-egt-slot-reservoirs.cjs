...
const explicitSources = String(process.env.EGT_RESERVOIR_SOURCES || '').split(path.delimiter).map(value => value.trim()).filter(Boolean);
...
function sourceFilesFor(gameKey) {
  const files = [];
  for (const file of explicitSources) {
    const absolute = path.resolve(__dirname, file);
    if (!fs.existsSync(absolute)) continue;
    try { if (readJson(absolute).gameKey === gameKey) files.push(absolute); } catch {}
  }
...
function discoverGameKeys() {
  ...
  for (const file of explicitSources) {
    const absolute = path.resolve(__dirname, file);
    if (!fs.existsSync(absolute)) continue;
    try { const gameKey = readJson(absolute).gameKey; if (gameKey) keys.add(gameKey); } catch {}
  }
  for (const dir of sourceDirs) {
